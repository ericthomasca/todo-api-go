...
func createTodo(w http.ResponseWriter, r *http.Request) {
	...
	todo := Todo{}
	err = conn.QueryRow(context.Background(), "UPDATE todo SET todo = $1, status = $2 WHERE id = $3 RETURNING *", todo.Todo, todo.Status, id).Scan(&todo.Id, &todo.Todo, &todo.Status, &todo.DateCreated)
	if err != nil {
		if err == pgx.ErrNoRows {
			http.NotFound(w, r)
		} else {
